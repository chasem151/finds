...
int mygrep(int argc, const char** argv) {
  int index;
  int c;
  opterr = 0;
    ...
    switch (c)
    {
      ...
      case 'f':
        ...
        if (*optarg != 'c' && *optarg != 'h' && *optarg != 'S') {
          printf("Try again. Invalid file extension.\n");
          return 1;
        }
        filext = optarg;
        break;
  ...
  printf("pathName = %s\nfFlag = %d\nfilext = %s\nlFlag = %d\nstring = %s\n\n", pathName, fFlag, filext, lFlag, string);
...
int regexLineParser(char* string, char* line, size_t length) {
  ...
  for(i =0; i < length; i++) {
    ...
    if (r + 1 < regexSize &&  string[r+1] == '*') {
      ...
      wildcardstar = 0;
      wildcardq = 0;
    } else if (wildcardq == 1 && line[i] == string[r-2]) {
      wildcardq = 0;
    } else if (wildcardstar == 1 && line[i] == string[r-2]) {
    ...
    case FTW_F:
      if (lFlag == 1) {
        if (S_ISREG(statptr->st_mode) != 0) {
          ...
          char* symLinkPath = malloc(PATH_MAX+1);
          if( (readlink(path, symLinkPath, PATH_MAX+1)) != -1){
